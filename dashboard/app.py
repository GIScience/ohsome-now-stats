...
app.layout = html.Div([
    ...
    dbc.Card(
        dbc.CardBody([
            dbc.Row([
                dbc.Col([
                    dash_table.DataTable([{col: df[col].sum() for col in cols}], [{"name": i, "id": i} for i in cols],id="baseTable")
                    ],width=12)
                    ],align='center'),
            html.Br(),
            dbc.Row([
...
def getSelectInterval(start_date:str,end_date:str,interval:str):
    ...
    if interval == "hourly":
        n_1 = 4
        n_2 = 4
        fun = lambda x: str(int(x.year)) + "-" + str(int(x.month)) + "-" +str(int(x.day))+ " " + str(int(x.hour))+":00"

    if interval == "minutely":
        n_1 = 5
        n_2 = 5
        fun = lambda x: str(int(x.year)) + "-" + str(int(x.month)) + "-" +str(int(x.day))+ " " + str(int(x.hour))+":" + str(int(x.minute))
